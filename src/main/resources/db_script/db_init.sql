...
VALUES (1, 'application.db_version', '14', now(), now(), false),         # for this database version
       (2, 'application.db_version_name', '0.4.4', now(), now(), false), # db version name
       ...
       (7, 'weight.protocol_version', '1', now(), now(), false);
...
(
    ...
    indivisible                bit(1)         not null default false comment 'Mark this slot can not be split',
    `create_time`              datetime       NOT NULL COMMENT 'time of create',
    `update_time`              datetime       NOT NULL COMMENT 'time of last update',
    `deleted`                  bit(1)         NOT NULL COMMENT 'delete flag',
    PRIMARY KEY (`id`)
...
(
    ...
    type             int(3)       not null default 1 comment 'Weight sensor type',
    `create_time`    datetime     NOT NULL COMMENT 'time of create',
    `update_time`    datetime     NOT NULL COMMENT 'time of last update',
    `deleted`        bit(1)       NOT NULL COMMENT 'delete flag',
    PRIMARY KEY (`id`)
