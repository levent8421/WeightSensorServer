drop database if exists db_weight_slot;
...
(
    ...
    zero_reference double       not null default 0 comment '零点参考',
    config_str     varchar(255) null comment '传感器配置',
...
(
    ...
    state         int(2)       not null default 0 comment '货道状态',
    create_time   datetime     not null comment '记录创建时间',
    update_time   datetime     not null comment '上次更新时间',
    deleted       bit(1)       not null comment '标记删除'
) engine = 'InnoDb'
...
insert into t_application_config (name, value, create_time, update_time, deleted) value ('application.db_version', '0.1.2', now(), now(), false);